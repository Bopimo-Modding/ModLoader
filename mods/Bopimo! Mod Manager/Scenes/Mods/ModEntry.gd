...
func set_mod(meta: Registry.GameData.ModData):
	...
	entry = load_data(metadata.load_path)
	if entry == {} :
		missing = true
		%Name.text = "MISSING"
	...
	%Creator.text = "By %s" % entry.creator
...
func load_data(path: String) -> Dictionary:
	...
	data.creator = config_file.get_value("Godot Mod", "creator")
	data.name = config_file.get_value("Godot Mod", "name")
	data.description = config_file.get_value("Godot Mod", "description")
	data.version = config_file.get_value("Godot Mod", "version")
